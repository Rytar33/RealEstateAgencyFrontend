...
const RoomEdit = ({ uuid }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const roomRes = await axios.get(`https://localhost:44329/api/v1.2/Room/${uuid}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(`https://localhost:44329/api/v1.2/Room`, formData, {
        headers: {
      ...
      if (res.status === 204)
        alert("Комната успешно изменена!");
    } catch (err) {
      setError(err.message);
    }
  };
