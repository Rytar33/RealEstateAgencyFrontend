...
const HostelEdit = ({ uuid }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch hostel data by UUID
        const hostelRes = await axios.get(`https://localhost:44329/api/v1.2/Hostel/${uuid}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(`https://localhost:44329/api/v1.2/Hostel`, formData, {
        headers: {
      ...
      if (res.status === 204)
        alert("Общежитие успешно изменена!");
    } catch (err) {
      setError(err.message);
    }
  };
